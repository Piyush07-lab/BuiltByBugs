...
function GitHubProfileSlide({ summary }) {
    ...
    return (
        <div className="flex min-h-auto md:min-h-60 flex-col gap-4">
            ...
            <p className="m-0 leading-relaxed text-muted">{user.bio || "No bio available."}</p>
            ...
            <a className="w-fit text-[0.9rem] font-extrabold text-accent no-underline hover:underline" href={user.html_url} target="_blank" rel="noreferrer">
                View GitHub profile
            </a>
        </div>
    );
            ...
            <div className="flex flex-col gap-2.5">
                {topRepos.map((repo) => (
                    <a className="flex items-center justify-between gap-4 border-b border-[#9eaedb]/10 pb-2.5 text-[#d7deed] no-underline hover:text-white" href={repo.html_url} target="_blank" rel="noreferrer" key={repo.id || repo.name}>
                        <span>{repo.name}</span>
                        <small className="text-subtle">{repo.language || "Unknown"} / {repo.stargazers_count || 0} stars</small>
                    </a>
                ))}
            </div>
