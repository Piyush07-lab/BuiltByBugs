...
function containsBannedWords(message) {
    ...
    return banned.some((word) => lowerMsg.includes(word.toLowerCase()));
}

function isValidHireRequest({ service: _service, name, email, details: message }) {
    name = name?.trim();
    email = email?.trim();
    ...
    if (!/^[a-zA-Z\s.'-]+$/.test(name)) return { ok: false, reason: "Invalid name" };

    if (email.length > 254) return { ok: false, reason: "Email too long" };

    if (message.length > 5000) return { ok: false, reason: "Message too long" };
...
function isValidContactRequest({ name, email, message, details }) {
    ...
    if (!/^[a-zA-Z\s.'-]+$/.test(name))
        return { ok: false, reason: "Invalid characters in name" };
...
export { isValidHireRequest, isValidContactRequest };
